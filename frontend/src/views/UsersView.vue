<template>
  <div class="pages container-fluid bg-black pb-5 ">
    <div class="container ">
      <div class="pt-4">
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <h5 class="text-start mt-2">User Password: </h5>
                <input v-model="payload.userPwd" type="text" :placeholder="user.userPwd" class="form-control">
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
    ...
    <table class="AdiCont bg-white table w-75 mt-5 text-center mx-auto" v-else>
      ...
      <tbody class="text-center mb-5">
        <tr v-for="user in filterUser" :key="user.userID">
          ...
          <td>{{ user.userRole }}</td>
          <td>
            <button class="btn bg-black text-white" data-bs-toggle="modal"
              :data-bs-target="'#edit' + user.userID">Edit</button>
          </td>
          <td>
            <button class="btn bg-black text-white" data-bs-toggle="modal"
              :data-bs-target="'#delete' + user.userID">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
